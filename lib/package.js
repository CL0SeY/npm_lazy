...
// uncached direct request (non-npm)
Package.proxyExternal = function(req, res) {
  logger.log('Querying the registry (external):', req.url);
  request(req).pipe(res).once('error', function(e) {
...
Package.proxy = function(req, res) {
  ...
  logger.log('Querying the registry (not cached):', remoteUrl + req.url.substr(1));
